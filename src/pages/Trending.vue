  ...
  <div class="min-h-screen text-gray-100 pb-10">
    <div class="container mx-auto px-4 md:px-8 pt-10">
      ...
      <div class="flex justify-between items-end mb-8">
        <h1 class="text-2xl md:text-3xl text-(--color-text) font-extrabold border-l-4 border-red-600 pl-3">
          <i class="fa-solid  fa-arrow-trend-up"></i> Trending Movies (Page {{ currentPage }})
        </h1>
      </div>
      ...
      <div v-else-if="movies.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
        <div
          v-for="movie in movies"
          :key="movie.id"
          class="group bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-red-900/20 hover:shadow-2xl transition duration-300 transform hover:-translate-y-1"
        >
